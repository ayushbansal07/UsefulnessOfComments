...
def getScopeForAFile(file, results):
	...
	while i < len(comments):
		...
		comment = comments[i]
		start_line = int(comment[2]) - 1
		end_line = int(comment[3]) - 1
		...
		j = i+1
		while j < len(comments) and comments[j][2] - 1 == end_line + 1:
		...
		if j < len(comments):
			for ind in range(i, j):
				scope[ind] = (start_line + 1, min(block_end, int(comments[j][2]) - 1))
		else:
			for ind in range(i, j):
